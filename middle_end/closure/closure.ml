...
let simplif_prim_pure ~backend fpc p (args, approxs) dbg =
  ...
  (* Block construction *)
  | Pmakeblock(tag, Immutable, _kind, _hint), _, _ ->
      let field = function
        | Value_const c -> c
        | _ -> raise Exit
      in
...
let simplif_prim ~backend fpc p (args, approxs as args_approxs) dbg =
  ...
  then simplif_prim_pure ~backend fpc p args_approxs dbg
    ...
    let approx =
      match p with
      | P.Pmakeblock(_, Immutable, _kind, _hint) ->
    ...
    | (p1 :: pl, a1 :: al) ->
        if is_substituable ~mutable_vars a1 then
          ...
          let u1, u2 =
            ...
            | "*opt*",
              Uprim(P.Pmakeblock(0, Immutable, kind, hint), [a], dbg) ->
                ...
                a, Uprim(P.Pmakeblock(0, Immutable, kind, hint),
                         [Uvar (VP.var p1')], dbg)
            | _ ->
                a1, Uvar (VP.var p1')
          in
