...
let primitive (p : Clambda_primitives.primitive) (args, approxs)
  ...
  match p with
  | Pmakeblock(tag_int, Asttypes.Immutable, shape, hint) ->
    ...
    Prim (Pmakeblock(tag_int, Asttypes.Immutable, Some shape, hint), args, dbg),
  ...
  | Pmakearray(_, _) when is_empty approxs ->
    Prim (Pmakeblock(0, Asttypes.Immutable, Some [], Lambda.Default_memory),
          [], dbg),
