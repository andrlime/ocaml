...
module Constr : sig
  ...
  let apply constr t =
    ...
    Lprim (Pmakeblock (constr.tag, constr.flag, constr.shape, constr.hint),
...
let rec choice ctx t =
  ...
  and choice_makeblock ctx ~tail:_ (tag, flag, shape, hint) blockargs loc =
    ...
    | Choice.No_tmc_call args ->
        Choice.lambda @@ Lprim (Pmakeblock (tag, flag, shape, hint), args, loc)
    | Choice.Ambiguous { explicit; subterms = ambiguous_subterms } ->
        ...
        *)
        let term_choice =
          ...
          Lprim (Pmakeblock(tag, flag, shape, hint), args, loc)
        in
        { term_choice with
  ...
  and choice_prim ctx ~tail prim primargs loc =
    ...
    | Pmakeblock (tag, flag, shape, hint) ->
        choice_makeblock ctx ~tail (tag, flag, shape, hint) primargs loc
